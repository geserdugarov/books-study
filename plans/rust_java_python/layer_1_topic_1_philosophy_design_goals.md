...
| 5.2 | 30 min | **Java versioning.** Read Horstmann Ch.1 §1.4 "A Short History of Java" (covers the release timeline). Read Evans Ch.1 pp. 10–15 (release model). Read the Oracle roadmap and JEP 12 online. Key insight (per Evans): 6-month feature releases since Java 10 (2018); LTS releases (Java 8 retroactively, then 11, 17) came roughly every three years as of book writing, with discussions in late 2021 about shifting to a two-year LTS cadence. Per the current Oracle Java SE support roadmap, that two-year cadence has since been adopted: Java SE 8, 11, 17, 21, and 25 are now LTS, with Java SE 29 planned next. Preview features allow experimentation. Binary compatibility since 1996 — extreme backwards compat. |
