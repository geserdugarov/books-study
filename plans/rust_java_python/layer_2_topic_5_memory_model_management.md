...
| Java | Goetz et al (2006) — *Java Concurrency in Practice* | Ch.16 pp. 337–352 | Java Memory Model — happens-before in detail, publication and initialization safety for `final` fields, safe publication of effectively immutable objects |
...
| 3.3 | 20 min | **Evans and Goetz on the Java Memory Model.** Read Evans Ch.5 pp. 119–140 (Java Memory Model, happens-before, concurrency through bytecode) for the modern overview, then read Goetz Ch.16 pp. 337–352 (JMM in depth: §16.1 What is a memory model and why would I want one?; §16.2 Publication; §16.3 Initialization safety for `final` fields) for the formal model. Key insight: the Java Memory Model (JMM) defines visibility rules for shared memory across threads — when one thread's write to a field becomes visible to another thread — via the happens-before partial order. Goetz's §16.3 is the load-bearing publication reference: the JMM's *initialization safety* guarantee lets correctly constructed `final` fields be safely shared across threads without synchronization, which is what makes immutable objects allocated by the GC safe to publish. Note that the JMM is about visibility/publication/initialization, not about deciding which objects the GC may reclaim — that decision is purely a reachability question (tracing from GC roots, computed at safepoints). Safepoints — special points where all application threads are stopped — are how the JVM gets a consistent heap snapshot for that reachability scan. |
...
| 3 | GC roots & reference counting | Oaks 121–152, Beckwith 20–42, Evans 119–140 + 207–215, Goetz 337–352, Shaw 200–219, Martelli 429–441, Ramalho 220–228 | Oracle GC Tuning Guide, JEP 248/333/189, CPython GC design docs, PEP 442 |
