...
| Java | Horstmann (2024) — *Core Java, Vol. I* | Ch.11 (Annotations) | Annotations — bytecode engineering, source-level annotation processing |
...
| Python | Ramalho (2022) — *Fluent Python* | Ch.9 pp. 310–311 ("Comparing Bytecodes" sidebar) | Bytecode analysis with the `dis` module — comparing `LOAD_FAST` vs `LOAD_GLOBAL` to see how the compiler classifies a name as local vs global |
...
| Java | Evans & Gough (2024) — *Optimizing Cloud Native Java* | Ch.6 pp. 163–167 (AOT pp. 163–164, Quarkus pp. 164–166, GraalVM p. 167) | Ahead-of-time compilation, Quarkus native mode, GraalVM native-image and its reflection-configuration constraints |
...
| 2.3 | 25 min | **Linking and build profiles.** Read Matthews Ch.2 pp. 27–31 (linking to C libraries, binary distribution, cross-compilation, static linking). Read Blandy Ch.8 pp. 161–175 (crate structure, build profiles). Read The Cargo Book "Build Profiles" and The Rust Reference "Linkage" online. Key insight: Rust defaults to static linking (the binary includes all Rust dependencies). Dynamic linking is used for system libraries (libc). The `musl` target enables fully static binaries with no system library dependencies. Debug builds (`dev` profile) include debug symbols and disable optimizations; release builds enable `-O3` by default, while LTO (link-time optimization) is opt-in and requires `[profile.release] lto = true` in `Cargo.toml`. The difference between debug and release performance can be 10–40x. |
...
| 3.3 | 25 min | **Type erasure, invokedynamic, and modern internals.** Read Bloch's Effective Java generics discussion on type erasure. Read Evans Ch.17 pp. 571–607 (method invocation — virtual/interface/special/final, MethodHandles, invokedynamic, string optimization). Read Horstmann Vol. I Ch.11 (annotation processing and bytecode engineering). Key insight: Java generics are erased at compile time — `List<String>` becomes `List<Object>` in bytecode, with cast instructions inserted by the compiler. `invokedynamic` (added in Java 7) allows the JVM to defer method resolution to runtime, enabling lambda expressions, string concatenation optimization, and dynamic language support. |
| 3.4 | 20 min | **GraalVM and ahead-of-time compilation.** Read Oaks Ch.4 pp. 116–120 ("The GraalVM" / "GraalVM Native Compilation" — JIT-vs-AOT tradeoffs, startup/footprint, peak-throughput vs HotSpot C2), Beckwith Ch.8 pp. 273–306 ("Accelerating Time to Steady State" — JVM start-up phases, CDS, AOT, GraalVM native-image, CRIU/CRaC, serverless optimization), and Evans/Gough Ch.6 pp. 163–167 (AOT, Quarkus native mode, GraalVM native-image with reflection-configuration constraints). Then read the GraalVM compiler and Native Image documentation, JEP 295 (AOT compilation), and Oracle's CDS documentation online. Key insight: GraalVM provides an alternative JIT compiler (written in Java itself) that can also compile Java applications ahead-of-time to native binaries via `native-image`. This eliminates JVM warmup and reduces startup time from seconds to milliseconds — critical for serverless/CLI use cases. The tradeoff: native-image has restrictions (limited reflection — must be statically configured, no dynamic class loading) and peak throughput may be lower than HotSpot C2 for long-running applications. CDS (Class Data Sharing) is a less radical approach that pre-loads class metadata to reduce startup time. |
...
| 2 | Rust compilation pipeline | Klabnik Ch.10 + 273–294, Blandy 161–175 + 235–263, Matthews 27–31, McNamara 137–173 | Rustc Dev Guide (HIR, MIR, LLVM backend), Rust Reference linkage, Cargo build profiles, MIR blog post |
| 3 | JVM compilation & execution | Evans 81–117 + 207–246 + 571–607, Horstmann Vol.II 114–126, Vol.I Ch.11, Bloch generics, Oaks 89–120, Beckwith 1–42 + 273–306, Evans/Gough 163–167 | JVM Spec Ch.4–6, HotSpot JIT docs, GraalVM compiler + native-image, JEP 295, CDS docs |
| 4 | Python execution model | Martelli 429–441, Ramalho 310–311 + 695–720, Gorelick 21–64 + 161–211, Shaw 61–175 | CPython compiler design, `dis` module docs, PEP 3147, PEP 659, PEP 744, PEP 703, PyPy JIT docs |
| 5 | Runtime systems compared | Matthews 93–118, Evans 207–230 + 571–590, Martelli 429–441, McNamara 175–211, Oaks 116–120 + 121–201, Beckwith 273–306, Shaw 177–219 | Oracle GC tuning guide, G1/ZGC docs, Python `gc` module, CDS/AppCDS docs, PEP 703 |
...
| 7 | WASM, GraalVM & linking | Matthews 27–31, Blandy 525–583, Horstmann Vol.II 160–174, McNamara 328–363, Beckwith 273–336, Evans/Gough 163–167 | Rust+WASM book, wasm-bindgen, GraalVM native-image + Truffle, JEP 454, WebAssembly spec, Pyodide, PyO3 |
