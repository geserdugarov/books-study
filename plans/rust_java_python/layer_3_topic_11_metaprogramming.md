...
| Java | Bloch (2018) — *Effective Java* | Item 65 p. 282 | Prefer interfaces to reflection — discusses when reflection is and is not appropriate |
...
| Java | Horstmann (2024) — *Core Java, Vol. II* | Ch.8 §8.1–8.2 pp. 562–584 | Compiling and scripting: the `javax.tools` Compiler API (invoking the compiler, launching a compilation task, capturing diagnostics, reading source from memory) at §8.1 pp. 562–572, and the Scripting API for embedding script languages (JavaScript, Groovy) inside Java applications at §8.2 pp. 573–584 — programmatic code generation and dynamic execution on the JVM |
...
| Python | Martelli et al (2023) — *Python in a Nutshell* | Ch.17 §"The inspect Module" pp. 528–533 | Runtime introspection with the `inspect` module: `signature` (parameter inspection), `getsource` (source retrieval), `getmembers` (member listing with predicates), `currentframe`/`stack` (frame inspection), and related debugging-oriented predicates |
...
| Python | Beazley (2021) — *Python Distilled* | Ch.5 §5.18–5.22 pp. 124–135 | Decorators (§5.18), function introspection / attributes / signatures via `inspect.signature()` (§5.20), environment inspection / frame access (§5.21), dynamic code execution and creation with `exec`/`eval`/`compile` (§5.22) |
| Python | Beazley (2021) — *Python Distilled* | Ch.7 §§7.21–7.22, 7.26, 7.28–7.31 pp. 194–221 | Class metaprogramming machinery: class decorators (§7.21 p. 194), supervised inheritance via `__init_subclass__` (§7.22 p. 197), proxies/wrappers/delegation (§7.26 p. 208), descriptors (§7.28 p. 211), the class definition process (§7.29 p. 215), dynamic class creation (§7.30 p. 216), and metaclasses (§7.31 p. 217) |
| Python | Beazley & Jones (2013) — *Python Cookbook* | Ch.9 pp. 329–396 | Metaprogramming recipes: decorators (with/without arguments, as classes, preserving metadata via `functools.wraps`, unwrapping), class-method decorators, metaclasses (instance creation control, attribute order capture, optional arguments, coding-convention enforcement), descriptor patterns, multiple dispatch via annotations, programmatic class creation, executing code with local side effects, parsing/analyzing Python source via `ast`, disassembling bytecode |
| Python | Beazley & Jones (2013) — *Python Cookbook* | §10.11–10.12 pp. 412–430 | Import-hook metaprogramming: loading modules from a remote machine using `sys.meta_path` finders and loaders, patching modules on import for instrumentation |
...
- **Annotation-processor toolchain integration beyond Horstmann's example** — Horstmann Ch.11 §11.5 walks through `AbstractProcessor`, `@SupportedAnnotationTypes`, `RoundEnvironment`, multi-round processing, and source generation via `Filer.createSourceFile`, but does not cover packaging a processor as a JAR with `META-INF/services/javax.annotation.processing.Processor` for auto-discovery, IDE integration (IntelliJ/Eclipse annotation-processor configuration), Maven/Gradle build wiring (`maven-compiler-plugin`'s `<annotationProcessorPaths>`, Gradle's `annotationProcessor` configuration), or incremental processing (Gradle's `isolating`/`aggregating` classifications); the Oracle Annotation Processing tutorial, build-tool docs, and the `javax.annotation.processing` Javadoc are needed
...
- **Higher-level Java bytecode-engineering libraries (Byte Buddy, cglib) and deeper ASM coverage** — Horstmann Ch.11 §11.6 demonstrates ASM by inserting logging into annotated methods (`AdviceAdapter`, `ClassReader`/`ClassWriter`) and §11.6.2 "Modifying Bytecodes at Load Time" walks through a `java.lang.instrument` agent example (`premain`, `Instrumentation`, the `-javaagent` switch); Evans Ch.4 covers bytecode at the conceptual level. But no owned book covers Byte Buddy's higher-level fluent API (`new ByteBuddy().subclass(...).method(...).intercept(...).make()`), cglib's proxy-generation model, ASM's `Tree` API, frame computation/StackMapTable details, or production-grade agent patterns; the ASM Guide (online PDF) and Byte Buddy documentation are needed for these
- **`ast.NodeTransformer` walkthroughs and `ast.unparse` round-trips beyond Cookbook §9.24** — Beazley & Jones cover `ast.parse()` and analysis at the recipe level, but neither owned book provides a full visitor/transformer rewrite tutorial or the Python 3.9+ `ast.unparse` source-regeneration story; the Python `ast` module documentation and the Green Tree Snakes tutorial fill this gap
- **`importlib` finder/loader protocol formal specification** — Beazley & Jones Cookbook §10.11–10.12 show import-hook recipes, but the formal `MetaPathFinder` / `PathEntryFinder` / `Loader` protocols and the modern `importlib.abc` hierarchy require the Python `importlib` documentation and PEP 302 / PEP 451
...
| 4 | Reflection and runtime introspection | Horstmann 80–81; Evans 81–117, 571–607; Bloch 282; Ramalho 835–850; Beazley Distilled 129–135; Martelli Ch.17 528–533 | Oracle Reflection tutorial, `java.lang.reflect` docs, Python `inspect` docs, `std::any` docs |
...
| 8 | AST manipulation, bytecode, synthesis | Shaw 91–175; Evans 90–111; Horstmann Vol. I 148–149; Horstmann Vol. II 562–584; Beazley & Jones Cookbook 388–395 | Python `ast` docs, Green Tree Snakes, `syn`/`quote` docs, ASM Guide, Byte Buddy tutorial |
