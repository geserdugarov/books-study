...
| Python | Ramalho (2022) — *Fluent Python* | Ch.22 pp. 835–878 | Dynamic Attributes and Properties — `__getattr__`/`__setattr__`/`__delattr__` for computed attributes, properties via `@property` and the descriptor protocol it builds on, flexible object creation via `__new__`, attribute shadowing and the invalid-attribute-name problem |
| Python | Ramalho (2022) — *Fluent Python* | Ch.23 pp. 879–906 | Attribute Descriptors — data vs non-data descriptors, `__get__`/`__set__`/`__delete__`/`__set_name__` (PEP 487), descriptor usage patterns, the role of descriptors in implementing properties, methods, and `classmethod`/`staticmethod` |
| Python | Ramalho (2022) — *Fluent Python* | Ch.24 pp. 907–956 | Class Metaprogramming — classes as objects, `type` as a class factory, class decorators, `__init_subclass__` (PEP 487) for declarative subclass customization, `__class_getitem__`, metaclasses and their use cases, the class definition process end-to-end |
...
| Python | Beazley (2021) — *Python Distilled* | Ch.7 pp. 153–223 | Classes and Object-Oriented Programming — class definition (§7.2), inheritance and `super()` (§7.7), multiple inheritance and MRO with mixins (§7.19), ABCs and interfaces (§7.18), operator overloading and protocols (§7.6), properties (§7.17), type-based dispatch (§7.20), class decorators (§7.21), supervised inheritance / `__init_subclass__` (§7.22), object life cycle and weak references (§7.23–7.24), `__slots__` (§7.27), descriptors (§7.28), class-definition process and metaclasses (§7.29–7.31) |
| Python | Beazley & Jones (2013) — *Python Cookbook* | Ch.8 pp. 243–328 | Classes and Objects — `__str__`/`__repr__`/`__format__`, context-management protocol, `__slots__`, encapsulation/properties, descriptors, lazy properties, ABCs, custom containers, mixins, comparison operations, managing cyclic data structures |
| Python | Beazley & Jones (2013) — *Python Cookbook* | Ch.9 pp. 329–395 | Metaprogramming — decorators (preserving metadata, with arguments, defined as classes), class decorators, metaclasses (controlling instance creation §9.13, capturing class attribute definition order §9.14, optional metaclass arguments §9.15, enforcing coding conventions §9.17, programmatic class creation §9.18, initializing class members at definition time §9.19), multiple dispatch via annotations §9.20 (the book predates PEP 487, so `__init_subclass__` patterns are not covered) |
...
- **Python descriptor protocol in complete depth** — Martelli covers descriptors briefly in Ch.4, Ramalho Ch.22 (Dynamic Attributes) plus Ch.23 (Attribute Descriptors) gives the data-vs-non-data descriptor distinction and `__get__`/`__set__`/`__delete__`/`__set_name__` end-to-end, Slatkin covers descriptors in Item 60, and Beazley Ch.7 plus Beazley & Jones Cookbook Ch.8 add additional worked examples; the gap that remains is the precise instance-lookup algorithm in `object.__getattribute__` (how MRO traversal interleaves with descriptor lookup; for class-object lookup the analogous algorithm lives in `type.__getattribute__`) and how `property`/`classmethod`/`staticmethod` are implemented in C — these require the Python Data Model documentation, the Descriptor HowTo Guide, and CPython's `Objects/descrobject.c`
- **Design patterns expressed idiomatically across all three languages** — none of the owned books provide a cross-language treatment of how Rust eliminates some patterns entirely (Visitor via `enum` + `match`, Strategy via closures), how Java's sealed classes reshape patterns, and how Python uses duck typing and first-class functions to simplify others (the 2024 edition of Naftalin & Wadler dropped the 2006 edition's "Design Patterns" chapter); this requires supplemental design patterns resources
...
- **Python metaclass mechanics and their interaction with `__init_subclass__`** — Martelli covers metaclasses in Ch.4, Ramalho Ch.24 covers `__init_subclass__`, class decorators, `__class_getitem__`, and metaclasses with worked examples, Slatkin covers `__init_subclass__` in Items 62–63 and metaclass alternatives in Item 66, and Beazley & Jones Cookbook Ch.9 (recipes 9.13–9.20) supplies additional metaclass examples covering instance-creation control, attribute-definition-order capture, optional metaclass arguments, coding-convention enforcement, programmatic class creation, class-member initialization at definition time, and multiple dispatch via annotations; what remains uncovered is the C-level implementation of the class-construction protocol — this requires the CPython source and PEP 487
...
| 1.3 | 25 min | **Python: classes and the data model.** Read Ramalho Ch.1 pp. 3–20 (Python data model, special methods). Read Martelli Ch.4 pp. 115–135 (classes and instances, `__init__`, `__new__`, attribute access). Read Beazley §7.1–7.5 (Objects, the `class` statement, Instances, Attribute Access, Scoping Rules) within Ch.7 pp. 153–223 — class definition mechanics, the role of `type` as the default metaclass, `__init__` vs `__new__`, attribute lookup order. Read Viafore Ch.10 pp. 135–145 (class anatomy, invariants). Key insight: Python classes are dynamic namespaces. `__init__` is not a constructor — it is an initializer; `__new__` is the actual constructor. Every instance attribute access goes through `type(obj).__getattribute__(obj, name)` — which for normal classes is `object.__getattribute__`; class-object attribute access goes through `type.__getattribute__` instead. Special methods (`__repr__`, `__str__`, `__eq__`, `__hash__`) define how objects behave with built-in functions and operators. Class bodies execute at definition time, creating a namespace dict that becomes the class's `__dict__`. There are no access modifiers — encapsulation relies on the name-mangling convention (`_private`, `__mangled`) and developer discipline. |
...
| 5.3 | 25 min | **Python: attribute lookup, `__dict__`, and MRO-based dispatch.** Read Martelli Ch.4 pp. 145–169 (attribute access, `__getattribute__`, descriptors, `__getattr__`), Ramalho Ch.22 pp. 835–878 (Dynamic Attributes and Properties — how `__getattr__`/`__setattr__` interpose on the lookup chain, property as a descriptor-based abstraction) and Ramalho Ch.23 pp. 879–906 (Attribute Descriptors — data vs non-data descriptors, `__get__`/`__set__`/`__delete__`/`__set_name__`, the canonical patterns), and Beazley §7.4 (Attribute Access) plus §7.25 (Internal Object Representation and Attribute Binding) and §7.28 (Descriptors) within Ch.7 pp. 153–223 (broad coverage of attribute access, the descriptor protocol, and how descriptors underpin method binding; the precise step-by-step `object.__getattribute__` / `type.__getattribute__` algorithm is *not* given here — for that, follow up with the Python Descriptor HowTo Guide, as flagged in Coverage Gaps). Cross-reference Beazley & Jones Cookbook §8.9 (creating a new kind of class or instance attribute) and §8.10 (lazily computed properties) within Ch.8 for descriptor-based recipes. Read the Python Descriptor HowTo Guide online. Key insight: Python method dispatch is a dictionary lookup chain. When you call `obj.method()`, Python calls `type(obj).__getattribute__(obj, 'method')`, which searches: (1) data descriptors in `type(obj).__mro__`, (2) `obj.__dict__`, (3) non-data descriptors in `type(obj).__mro__`. This is O(n) in MRO depth, not O(1) like vtable dispatch. CPython optimizes with type attribute caches (version-tagged) — the cache is invalidated when a class is modified. Despite caching, Python's dispatch is fundamentally slower than vtable-based dispatch but offers maximum flexibility: methods can be added, removed, or replaced at runtime on any class or instance. |
...
| 6.3 | 25 min | **Python: C3 linearization and super().** Read Ramalho Ch.14 pp. 497–518 (MRO, C3 linearization, `super()`, cooperative multiple inheritance) and Beazley §7.7 (Inheritance) plus §7.19 (Multiple Inheritance, Interfaces, and Mixins) within Ch.7 pp. 153–223 (the single linear list per class, the rules C3 enforces, examples of failures). Read Slatkin Ch.7 pp. 235–239 (Item 53: initialize parent classes with `super()`). Read Michele Simionato's "The Python 2.3 Method Resolution Order" online. Key insight: Python's C3 linearization algorithm produces a consistent method resolution order for multiple inheritance that respects two constraints: (1) subclasses come before superclasses, (2) the order of base classes in the class definition is preserved. If no consistent ordering exists, Python raises `TypeError` at class creation time. `super()` delegates to the next class in the MRO, not the parent class — this enables cooperative multiple inheritance where each class calls `super()` to chain behavior. In a diamond `D(B, C)` where both `B` and `C` inherit from `A`, the MRO is `[D, B, C, A]` — `B.method()` calling `super().method()` goes to `C`, not `A`. |
...
|---------|-------|-----------------|----------------------|
| 1 | Classes, structs, objects | Klabnik 85–102, Blandy 193–209, McNamara 77–95, Horstmann 55–68, Bloch 73–90, Ramalho 3–20, Martelli 115–135, Beazley 153–223 (§7.1–7.5), Viafore 135–145 | Rust Reference implementations, JEP 395 (Records), Python Data Model docs |
...
| 5 | Dynamic dispatch: vtables, trait objects, virtual methods | Klabnik 375–396, Gjengset 25–35, Evans 95–117 + 571–590, Martelli 145–169, Ramalho 835–906 (Ch.22 dynamic attributes + Ch.23 descriptors), Beazley 153–223 (§7.4, §7.25, §7.28 descriptors), Beazley & Jones 243–328 | Rustonomicon DSTs, Shipilev Anatomy Quark #16, JVM Spec invokevirtual |
| 6 | Method resolution order and coherence | Gjengset 30–50, Horstmann 74–82, Ramalho 497–518, Beazley 153–223 (§7.7, §7.19 MRO/mixins), Slatkin 235–239 | Rust RFC 2451, JLS 15.12 + 9.4, Simionato MRO article |
...
| 10 | Synthesis: design patterns | Slatkin 260–264 + 285–298, Ramalho 907–956 (Ch.24 class metaprogramming, `__init_subclass__`, metaclasses), Beazley & Jones 329–395 (decorators, metaclasses, multiple dispatch recipes), all previous sessions | Rust Design Patterns book, Refactoring Guru |
