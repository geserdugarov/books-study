...
| Java | Naftalin & Wadler (2024) — *Java Generics and Collections* | Chs. 4–13 | Complete coverage of the Java Collections Framework: main interfaces (Ch.4), preliminaries including `Iterable` and iterators (Ch.5), the `Collection` interface (Ch.6), the `SequencedCollection` interface for JEP 431 / Java 21+ — `SequencedCollection`, `SequencedSet`/`NavigableSet`, `SequencedMap`/`NavigableMap` (Ch.7), Sets (Ch.8), Queues (Ch.9), Lists (Ch.10), Maps (Ch.11), the `Collections` utility class (Ch.12), and guidance for choosing implementations (Ch.13) — with design rationale, performance characteristics, and generic typing throughout |
...
| Java | Goetz (2006) — *Java Concurrency in Practice* | Ch.5 pp. 79–110 | Building Blocks: synchronized collections and their iteration hazards (§5.1), concurrent collections — `ConcurrentHashMap`, `CopyOnWriteArrayList/Set`, weakly-consistent iterators (§5.2), blocking queues and the producer–consumer pattern — `BlockingQueue` implementations including `LinkedBlockingQueue`, `ArrayBlockingQueue`, `PriorityBlockingQueue`, and `SynchronousQueue` (§5.3) |
| Java | Valeev (2024) — *100 Java Mistakes and How to Avoid Them* | Ch.8 pp. 213–248 | Collections and maps: searching with wrong element type (Mistake 69), null in null-hostile collections (71), null map values (72), unmodifiable-collection writes (73), mutable keys (74), HashMap/HashSet encounter order (75), concurrent modification during iteration (76), `List.remove()` overload mix-ups and skipped elements (77–78), reading during `removeIf` (79), concurrent modification in `Map.computeIfAbsent` (80), iterator-contract violations (81) |
| Java | Valeev (2024) — *100 Java Mistakes and How to Avoid Them* | Ch.9 §9.2–9.5 pp. 251–260 | Stream API mistakes: side effects inside a Stream chain (Mistake 83), consuming a stream twice (84), null values where streams disallow them (85), violating the contract of Stream operations such as ordering and statelessness (86) |
| Java | Oaks (2020) — *Java Performance* | Ch.12 pp. 363–411 | Java SE API performance tips, including Java Collections API performance — synchronized vs unsynchronized collections, collection sizing and resizing cost, memory efficiency (pp. 392–397) — and Stream/filter performance and laziness (pp. 399–402); also lambdas vs anonymous classes, strings, buffered I/O, classloading (CDS), serialization |
...
| Python | Beazley & Jones (2013) — *Python Cookbook* | Ch.1 pp. 1–36 | Data structures and algorithms recipes: unpacking, `deque`, `heapq`, `defaultdict`, `OrderedDict`, `Counter`, `ChainMap`, sorting, filtering, grouping, dictionary subset/combination, multi-mapping |
| Python | Beazley & Jones (2013) — *Python Cookbook* | Ch.4 pp. 113–140 | Iterators and generators recipes: iterator protocol, generator functions with state, `itertools.islice`/`combinations`/`permutations`/`chain`, `enumerate`/`zip`, data processing pipelines, flattening nested sequences, merged sorted iterables |

### Coverage Gaps

The owned books **do not** cover:

- **OpenJDK-source-level internals of Java collections** — Horstmann and Naftalin & Wadler cover the visible implementation behaviour (hash bucket layout and treeification thresholds since Java 8, load factor and rehashing, red-black tree balancing, array growth, list/deque backing structures), but exact internal types (`HashMap.Node` vs `TreeNode` layout, `tableSizeFor` rounding, `MIN_TREEIFY_CAPACITY`/`TREEIFY_THRESHOLD`/`UNTREEIFY_THRESHOLD` constants, `ConcurrentHashMap` bin-state encoding) require the OpenJDK source and the `java.util` Javadoc implementation notes
...
| 5.2 | 30 min | **Java: Stream API, Collectors, and Optional.** Read Horstmann Ch.1 (Core Java II) pp. 30–39 (collecting results, grouping, partitioning, downstream collectors, reduction, primitive streams). Read Bloch Ch.7 Items 45–47 (streams judiciously, side-effect-free functions, `Collection` vs `Stream` return). Read Valeev Ch.9 §9.2–9.5 pp. 251–260 (Stream API mistakes: side effects in a chain, consuming a stream twice, null pitfalls, violating Stream-operation contracts such as ordering and statelessness). Key insight: Java's `Collectors` class provides a rich vocabulary: `toList()`, `toSet()`, `toMap()`, `groupingBy()`, `partitioningBy()`, `joining()`, `summarizingInt()`. Downstream collectors compose: `groupingBy(City::country, counting())` or `groupingBy(City::country, mapping(City::name, toList()))`. `Optional<T>` replaces null returns in stream operations — `findFirst()`, `findAny()`, `min()`, `max()` all return `Optional`. `reduce(identity, accumulator)` is Java's fold. Primitive streams (`IntStream`, `LongStream`, `DoubleStream`) avoid boxing overhead with `mapToInt()`, `sum()`, `average()`. The Stream API deliberately avoids the term "lazy" — it uses "intermediate" vs "terminal" to describe the evaluation model. |
...
| 7.2 | 30 min | **Java: `ConcurrentHashMap`, blocking queues, `CopyOnWriteArrayList`, `EnumSet`/`EnumMap`.** Read Goetz Ch.5 pp. 79–110 (synchronized vs concurrent collections, blocking queues, producer-consumer pattern — the canonical treatment; note that Goetz describes the Java 5/6 segment design, since superseded). Read Evans Ch.6 pp. 169–205 (`ConcurrentHashMap`, `CopyOnWriteArrayList`, blocking queues). Read Bloch Ch.6 Items 36–37 (`EnumSet`, `EnumMap`). Read the `java.util.concurrent` package overview online. Key insight: `ConcurrentHashMap` on Java 8+ abandoned the Java 5/6/7 segmented-lock design (`Segment[]` of ~16 striped locks) in favour of per-bucket synchronization: reads are lock-free, and writes use a CAS to install the first node into an empty bin or `synchronized` on the bin's head node when the bin is non-empty, with treeification at high collision counts. The historical segment-stripe design only survives as a constructor argument (`concurrencyLevel`) kept for source compatibility. `CopyOnWriteArrayList` is optimized for read-heavy, write-rare workloads — every write creates a new internal array copy. `BlockingQueue` implementations (`ArrayBlockingQueue`, `LinkedBlockingQueue`, `PriorityBlockingQueue`) are the backbone of producer-consumer patterns. `EnumSet` uses a bit vector internally — operations are a single machine word AND/OR. `EnumMap` uses an array indexed by ordinal — no hashing overhead. `PriorityQueue` is a min-heap. `ConcurrentSkipListMap` is a concurrent sorted map based on skip lists. |
...
|---------|-------|-----------------|----------------------|
| 1 | Sequence collections | Klabnik 141–150, Blandy 359–375, Matthews 65–78, Horstmann I 117–123, Naftalin & Wadler Ch.7 + Ch.10, Ramalho 21–50, Gorelick 65–77, Slatkin 521–528, Beazley & Jones Cookbook Ch.1 pp. 1–36 | Rust `std::collections` docs, JEP 431, Python Wiki TimeComplexity |
...
| 3 | Iterator protocols | Klabnik 273–285, Blandy 321–335, Horstmann I 117–120, Naftalin & Wadler Ch.4 + Ch.5 §"Iterable and Iterators", Ramalho 593–625, Slatkin 77–90, Martelli 80–100, Beazley & Jones Cookbook Ch.4 pp. 113–140 | Rust `std::iter` docs, JLS 14.14.2, Python iterator types docs |
| 4 | Lazy evaluation | Blandy 335–357, Klabnik 285–294, Horstmann II 24–39, Bloch Items 42–48, Ramalho 625–656, Slatkin 173–200, Gorelick 97–107, Beazley & Jones Cookbook Ch.4 pp. 113–140 | Rust `Iterator` provided methods, `java.util.stream` package docs, PEP 255/289/380 |
| 5 | Functional combinators | Blandy 335–357, Klabnik 273–285, Horstmann II 30–39, Bloch Items 45–47, Slatkin 173–188, Ramalho 21–35, Martelli 247–265, Valeev 251–260 | `FromIterator` docs, `Collectors` Javadoc, PEP 709 |
| 6 | Ownership in collections | Blandy 321–335 + 359–365, Gjengset 19–35, Horstmann I 123–127, Bloch Items 45–47, Valeev 231–248, Slatkin 95–100, Ramalho 50–65 | Rustonomicon ownership, `Collections.unmodifiableList` Javadoc, Python `copy` docs |
| 7 | Specialized & concurrent collections | Blandy 380–389, Matthews 219–231, Goetz 79–110, Evans 169–205, Bloch Items 36–37, Slatkin 521–532 + 115–134, Ramalho 95–116 | `crossbeam`/`dashmap`/`rayon` docs, `java.util.concurrent` package, Python `queue`/`heapq`/`bisect` docs |
| 8 | Custom collections & iterators | Blandy 348–357, Klabnik 285–290, Naftalin & Wadler Ch.13 §"Customize Collections Using the Abstract Classes", Ramalho 397–430, Slatkin 257–264, Viafore 68–76 | `DoubleEndedIterator`/`FromIterator`/`Index` docs, `AbstractList` Javadoc, `collections.abc` docs |
| 9 | Performance & optimization | Matthews 219–231, Klabnik 290–294, Bloch Item 48, Evans 537–555, Oaks 392–402, Gorelick 65–107 | Rust `std::collections` perf table, Shipilev JVM Anatomy Quarks, Python Wiki TimeComplexity |
