...
| Java | Evans et al (2022) — *The Well-Grounded Java Developer* | Ch.6 pp. 169–205 | JDK Concurrency Libraries — `Future.get()` wrapping task exceptions in `ExecutionException` and a basic introduction to `CompletableFuture` (§6.8), the `Executors` framework and exception propagation from `ExecutorService` tasks (§6.9), blocking queues and interruption; functional-style error APIs (`exceptionally`/`handle`/`whenComplete`) require the external Baeldung article listed below |
| Java | Goetz et al (2006) — *Java Concurrency in Practice* | Ch.5 pp. 79–110 | Building Blocks — exception handling in synchronized collections, `ConcurrentHashMap` iterator behaviour, blocking-queue interruption semantics |
| Java | Goetz et al (2006) — *Java Concurrency in Practice* | Ch.6 pp. 113–134 | Task Execution — exceptions in tasks submitted to `ExecutorService`, `Future.get()` wrapping in `ExecutionException`, time-limited tasks and `CompletionService` |
| Java | Goetz et al (2006) — *Java Concurrency in Practice* | Ch.7 pp. 135–166 | Cancellation and Shutdown — interruption as the cooperative cancellation mechanism, `InterruptedException` handling rules, the poison pill pattern, `UncaughtExceptionHandler` for fire-and-forget tasks (§7.3 Handling abnormal thread termination), graceful shutdown of executors |
...
| Python | Beazley (2021) — *Python Distilled* | Ch.3 §3.4 pp. 64–74 | Exception hierarchy, `try`/`except`/`else`/`finally`, exceptions and control flow, defining new exceptions, chained exceptions (`raise ... from ...`), tracebacks, exception handling advice |
...
| Python | Hattingh (2020) — *Using Asyncio in Python* | Ch.3 pp. 21–73 | Async task exception handling — `asyncio.gather(return_exceptions=True)`, cancellation and `CancelledError`, graceful shutdown patterns (the book predates `TaskGroup`/`ExceptionGroup`; pair with PEP 654 for current patterns) |
...
- **Error handling in async/concurrent code across all three languages** — Goetz Ch.6–7 and Evans Ch.6 give a thorough pre-`StructuredTaskScope` Java treatment, and Hattingh Ch.3 covers pre-`TaskGroup` asyncio patterns; what remains uncovered by the owned books is how Rust's `tokio`/`async-std` handle errors in spawned tasks (`JoinError`, `JoinSet`) and how the modern `StructuredTaskScope` (JEP 453) / `asyncio.TaskGroup` + `ExceptionGroup` patterns compose — these require crate documentation, JEPs, and PEP 654
...
| Step | Time | Activity |
|------|------|----------|
| 8.1 | 30 min | **Python: ExceptionGroup and except*.** Read Martelli Ch.6 pp. 215–219 (ExceptionGroup, `except*` syntax). Read Hattingh Ch.3 pp. 21–73 for the pre-3.11 baseline: `asyncio.gather(return_exceptions=True)`, propagating `CancelledError`, and graceful-shutdown patterns. Then read PEP 654 (Exception Groups and except*) online in full. Key insight: `ExceptionGroup` (Python 3.11+) wraps multiple exceptions that occurred concurrently. `except*` is a new clause that matches subgroups of exceptions within a group — each `except*` handler can match a subset of the exceptions, and unmatched exceptions continue propagating. This was designed for `asyncio.TaskGroup` and other structured concurrency APIs where multiple concurrent tasks can each fail independently. Example: `async with asyncio.TaskGroup() as tg: ...` raises `ExceptionGroup` if any tasks fail. `except* ValueError as eg:` catches all `ValueError` instances in the group. Multiple `except*` clauses can all match on the same `ExceptionGroup` (unlike regular `except`, where only the first match runs). `ExceptionGroup` can be nested (group within a group). The `exceptiongroup` backport package provides this for Python 3.7+. The contrast with Hattingh's pre-`TaskGroup` patterns highlights what `ExceptionGroup` actually solves: the old `gather(return_exceptions=True)` returned a flat list mixing successes and exceptions, requiring the caller to demux manually; `TaskGroup` + `except*` provides exhaustive structured handling. |
| 8.2 | 30 min | **Java: exception handling in concurrent code.** Read Goetz Ch.6 pp. 113–134 (Task Execution — exceptions in tasks submitted to `ExecutorService`, `Future.get()` wrapping in `ExecutionException`, `CompletionService`, time-limited tasks) and Goetz Ch.7 pp. 135–166 (Cancellation and Shutdown — cooperative interruption in §7.1, the canonical `InterruptedException` handling rules in §7.1.2 "Interruption policies" and §7.1.3 "Responding to interruption", the poison-pill pattern, and §7.3 "Handling abnormal thread termination" introducing `UncaughtExceptionHandler`). Goetz §5.4 ("Blocking and interruptible methods") is the earlier introduction to those interruption semantics. Then read Evans Ch.6 pp. 169–205 (JDK concurrency libraries — `Future.get()` / `ExecutionException` and a basic `CompletableFuture` introduction in §6.8, modern `ExecutorService`/`Executors` patterns in §6.9; for `CompletableFuture.exceptionally`/`handle`/`whenComplete` see the Baeldung article). Read JEP 453 (Structured Concurrency) online. Read the Baeldung article on CompletableFuture exception handling online. Key insight: Java's `Future.get()` wraps the task's exception in `ExecutionException` — the caller must call `getCause()` to access the original exception. This is exception translation forced by the concurrency API. Goetz's §7.1.2–§7.1.3 establish the rules everyone still follows: never swallow `InterruptedException`, restore the interrupt status if you cannot propagate it. §7.3 then explains how to install an `UncaughtExceptionHandler` for tasks whose exceptions would otherwise vanish into the thread pool. `CompletableFuture` provides `exceptionally()`, `handle()`, and `whenComplete()` for functional-style error handling. Java 21's `StructuredTaskScope` (preview) provides structured concurrency: `ShutdownOnFailure` cancels sibling tasks when one fails and rethrows the first exception; `ShutdownOnSuccess` cancels remaining tasks when one succeeds. Unlike Python's `ExceptionGroup`, Java does not (yet) have a built-in mechanism for aggregating multiple concurrent failures — `StructuredTaskScope.ShutdownOnFailure` reports only the first failure. |
